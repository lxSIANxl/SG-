...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleBankrupt() {
        ...
        // 1. 먼저 데이터를 한 번 읽어서 쿨타임인지 확인합니다.
        playerRef.once('value').then((snapshot) => {
            const data = snapshot.val();
            
            // 데이터가 아예 없으면 초기화 대상 (쿨타임 체크 패스)
            if (data && data.bank && data.bank.bankruptTimestamp) {
                const bankruptTime = data.bank.bankruptTimestamp;
                
                // 쿨타임 체크
                if (now - bankruptTime < TEN_MINUTES_MS) {
                    const remainingMs = TEN_MINUTES_MS - (now - bankruptTime);
                    const remainingSeconds = Math.floor(remainingMs / 1000);
                    const minutes = Math.floor(remainingSeconds / 60);
                    const seconds = remainingSeconds % 60;
                    
                    showAlert(`파산 신청 쿨타임 중입니다.\n(남은 시간: ${minutes}분 ${seconds}초)`);
                    return; // 여기서 함수 종료 (트랜잭션 실행 안 함)
                }
            }

            // 2. 쿨타임이 아니면 트랜잭션 실행
            executeBankruptTransaction();
        }).catch((error) => {
            console.error("데이터 읽기 오류:", error);
        });
    }

    function executeBankruptTransaction() {
        playerRef.transaction((currentPlayerData) => {
            // 데이터가 없으면 기본 틀 생성
            ...
            // bank 객체가 없으면 생성
            if (!currentPlayerData.bank) {
                currentPlayerData.bank = { savings: 0, loan: 0 };
            }

            // --- 파산 로직 적용 ---
            
            // 1. 자산 초기화
            currentPlayerData.portfolio = {}; // 주식 전량 폐기
            currentPlayerData.cash = 3000000; // 회생 자금 300만
            
            // 2. 은행 상태 재설정
            currentPlayerData.bank.savings = 0; // 예금 몰수
            currentPlayerData.bank.savingsTimestamp = null;
            
            currentPlayerData.bank.loan = 5000000; // 빚 500만 추가
            
            // 3. 타임스탬프 찍기 (firebase 객체가 있는지 확인 필요)
            if (typeof firebase !== 'undefined') {
                const ts = firebase.database.ServerValue.TIMESTAMP;
                currentPlayerData.bank.loanTimestamp = ts;
                currentPlayerData.bank.loanRepayTimestamp = null;
                currentPlayerData.bank.bankruptTimestamp = ts; // 파산 쿨타임 시작
            } else {
                // firebase 객체 접근이 안될 경우 현재 시간 사용
                const now = Date.now();
                currentPlayerData.bank.loanTimestamp = now;
                currentPlayerData.bank.loanRepayTimestamp = null;
                currentPlayerData.bank.bankruptTimestamp = now;
            }
        ...
        .then((result) => {
            if (result.committed) {
                showAlert("파산 신청이 완료되었습니다.\n초기 자금 300만원과 빚 500만원이 설정되었습니다.");
                // UI 업데이트 함수가 있다면 여기서 호출
                // updateUI(); 
            } else {
                showAlert("파산 신청 처리에 실패했습니다. (데이터 충돌)");
            }
        })
        .catch((error) => {
            console.error("파산 트랜잭션 오류:", error);
    ...
    function updateStockPrices() {
        ...
        marketRef.child('stocks').transaction((currentMarketData) => {
            ...
            Object.keys(currentMarketData).forEach(ticker => {
                ...
                if (stock.price <= delistThreshold && stock.price > 1) {
                    // 조건 도달: 상장폐지 처리
    ...
    function checkDevTools() {
        ...
        if (widthDiff || heightDiff) {
            document.body.innerHTML = "<h1>⛔ 개발자 도구가 감지되었습니다.</h1>";
            // 창을 닫을 순 없지만, 구글로 보내버릴 순 있습니다.
